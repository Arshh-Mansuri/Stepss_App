...
class ShieldManager {
    ...
    func loadSelectionFromDatabase() {
        guard let data = AppGroup.sharedDefaults.data(forKey: DefaultsKey.selection) else {
            return
        }
        ...
        do {
            ...
            AppGroup.sharedDefaults.removeObject(forKey: DefaultsKey.selection)
        }
    }
    ...
    private func saveSelection() {
        do {
            ...
            AppGroup.sharedDefaults.set(data, forKey: DefaultsKey.selection)
        } catch {
            AppGroup.sharedDefaults.removeObject(forKey: DefaultsKey.selection)
        }
    }
    ...
    private enum DefaultsKey {
        static let selection = "RestrictedAppSelection"
    }
}
